...
require 'eluent/cli/commands/close'

RSpec.describe Eluent::CLI::Commands::Close do
  ...
  describe '--reason option' do
    it 'stores close reason' do
      run_command(atom_id, '--reason', 'Completed successfully')

      data = read_atom_from_file
      expect(data['close_reason']).to eq('Completed successfully')
    end
  end

  describe '--message alias' do
    it 'stores close reason via -m' do
      run_command(atom_id, '-m', 'Fixed in PR #42')

      data = read_atom_from_file
      expect(data['close_reason']).to eq('Fixed in PR #42')
    end
  end

  describe 'CONFLICT error' do
    before do
      # Make atom already closed
      File.write(
        File.join(root_path, '.eluent', 'data.jsonl'),
        <<~JSONL
          {"_type":"header","repo_name":"testrepo"}
  ...
  describe 'robot mode' do
    it 'outputs JSON with atom data' do
      ...
      expect(parsed['status']).to eq('ok')
      expect(parsed['data']['status']).to eq('closed')
    end
  end

  describe '--help' do
    it 'shows usage' do
      expect { run_command('--help') }.to output(/el close/).to_stdout
    end
